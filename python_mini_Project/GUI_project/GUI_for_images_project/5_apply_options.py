...
def merge_image():
    ...
    ##############################################################################
        ...
        # 이렇게로도 가능!
        widths, heights = zip(*image_sizes) # unzip 해서 바로 넣
        ...
        for idx, img in enumerate(images):
            # width 가 원본유지가 아닐 때 이미지 크기 조절!
            if img_width > -1:
                img = img.resize(image_sizes[idx])

            result_img.paste(img, (0, y_offset))
            y_offset += (img.size[1] + img_space) # height 값 + 사용자가 정한 간격
        ...
        # 포맷 옵션 처리
        file_name = "yosuniiiii." + img_format
